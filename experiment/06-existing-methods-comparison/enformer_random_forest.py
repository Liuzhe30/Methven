...
output_path = '../../model/prediction_results_tenfold_enformer/'
...
for model_size in model_size_list:
    ...
    for i in range(10):
        if(str(i+1)!= fold):
            data = pd.read_pickle(final_path + model_size + '_split' + str(i) + '.dataset')
    ...
    for i in range(train_data.shape[0]):
        sample_feature = []
        sample_feature += train_data['result_before'][i].flatten().tolist()
    ...
    for i in range(test_data.shape[0]):
        sample_feature = []
        sample_feature += test_data['result_before'][i].flatten().tolist()

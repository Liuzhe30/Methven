...
if __name__ == "__main__":
    ...
    # load datasets
    train_data = pd.read_pickle('datasets/final/dnabert/train_' + model_size + '.dataset')
    valid_data = pd.read_pickle('datasets/final/dnabert/valid_' + model_size + '.dataset')
    test_data = pd.read_pickle('datasets/final/dnabert/test_' + model_size + '.dataset')
    ...
    if(model_size == 'small'):
        ...
        batch_size = 32
        model = build_methven_middle()
    elif(model_size == 'large'):
        batch_size = 32
        model = build_methven_large()
    elif(model_size == 'huge'):
        batch_size = 32
        model = build_methven_huge()
